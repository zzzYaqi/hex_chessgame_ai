...
class TestRaveNode:
    ...
    @property
    def value(self, explore: float = RaveMeta.EXPLORATION, rave_const: int = RaveMeta.RAVE_CONST):
...
class TestRaveMCTS:
    ...
    def _get_best_node(self, node: TestRaveNode) -> TestRaveNode:
    ...
    def _expand(self, parent: TestRaveNode, state: State):
        children = []
        for move in state.get_valid_moves():
            children.append(TestRaveNode(move, parent))
        parent.add_children(children)

    def _simulate(self, state: State) -> tuple:
        """
    ...
    def _back_prop(self, node: TestRaveNode, next_player, winner, red_moves: list, blue_moves: list):
